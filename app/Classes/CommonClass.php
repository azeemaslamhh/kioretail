...
class CommonClass {
    ...
    public function getCourierReport($courier_id, $start_date = "", $end_date = "", $courier_status = "") {
        ...
        if ($result->count() > 0) {
            $saleRow = $result->select(DB::raw('sum(sales.grand_total) as total_sale_amount, count(sales.id) as total_sale_count, sum(sales.paid_amount) as paid_amount , sum(sales.courier_fee) as shipping_cost'))->first();
        ...
        ///echo "query = ".$result2->toSql(); exit;
        $shipping_cost = $result2->sum('sales.courier_fee');
