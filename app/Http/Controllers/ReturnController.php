...
class ReturnController extends Controller
{
    ...
    public function returnData(Request $request)
    {
        ...
        $q = Returns::leftjoin('customers', 'returns.customer_id', 'customers.id')
                ->leftjoin('billers', 'returns.biller_id', 'billers.id')
                ->leftjoin('warehouses', 'returns.warehouse_id', 'warehouses.id')
                ->leftjoin('users', 'returns.user_id', 'users.id')->select('returns.*')
                ->select('returns.*','warehouses.name as warehouse_name','billers.name as biller_name','customers.name as customer_name','billers.name as billers_name','billers.company_name as biller_company_name','billers.email AS billers_email','billers.phone_number as billers_phone_number','billers.address as billers_address','billers.city as billers_city','customers.name as customers_name','customers.city as customers_city','users.name as user_name','users.email as user_email');
                //"warehouse_name","biller_name"","biller_company_name","biller_email","biller_phone_number","biller_address","biller_city"","customer_name","customer_city","user_name","user_emial"
        if(empty($request->input('search.value'))) {
            $q ->whereDate('returns.created_at', '>=' ,$request->input('starting_date'))
            ...
            elseif($warehouse_id != 0)
                $q = $q->where('returns.warehouse_id', $warehouse_id);
            $returnss = $q->get();
        }
        else
        {
            ...
            if(Auth::user()->role_id > 2 && config('staff_access') == 'own') {
                ...
                $returnss =  $q->orwhere('returns.reference_no', 'LIKE', "%{$search}%")
        ...
        if(!empty($returnss))
        {
            ...
            foreach ($returnss as $key=>$returns)
            {
                ...
                $nestedData['key'] = $key;
                $nestedData['date'] = isset($returns->created_at)? date(config('date_format'), strtotime($returns->created_at->toDateString())):"";
                ...
                $nestedData['biller'] = $returns->biller_name;
                ...
                $nestedData['return'] = array( '[ "'.date(config('date_format'), strtotime($returns->created_at->toDateString())).'"', ' "'.$returns->reference_no.'"', ' "'.$returns->warehouse_name.'"', ' "'.$returns->billers_name.'"', ' "'.$returns->billers_company_name.'"', ' "'.$returns->billers_email.'"', ' "'.$returns->billers_phone_number.'"', ' "'.$returns->billers_address.'"', ' "'.$returns->billers_city.'"', ' "'.$returns->customers_name.'"', ' "'.$returns->customers_phone_number.'"', ' "'.$returns->customers_address.'"', ' "'.$returns->customers_city.'"', ' "'.$returns->id.'"', ' "'.$returns->total_tax.'"', ' "'.$returns->total_discount.'"', ' "'.$returns->total_price.'"', ' "'.$returns->order_tax.'"', ' "'.$returns->order_tax_rate.'"', ' "'.$returns->grand_total.'"', ' "'.preg_replace('/[\n\r]/', "<br>", $returns->return_note).'"', ' "'.preg_replace('/[\n\r]/', "<br>", $returns->staff_note).'"', ' "'.$returns->user_name.'"', ' "'.$returns->user_email.'"', ' "'.$nestedData['sale_reference'].'"', ' "'.$returns->document.'"', ' "'.$currency_code.'"', ' "'.$returns->exchange_rate.'"]'
                );
                $data[] = $nestedData;
            }
        }
    ...
    public function store(Request $request)
    {
        ...
        if(!isset($data['total_qty']) || $data['total_qty']<1){
            return redirect('return-sale')->withErrors(['message' => 'You cannot return 0 product. select atleast one item']);
        }
        //return dd($data);
    ...
    public function productReturnData($id)
    {
        ...
        foreach ($lims_product_return_data as $key => $product_return_data) {
            ...
            $product_return[0][$key] = isset( $product->name)? $product->name . ' [' . $product->code . ']':"";
