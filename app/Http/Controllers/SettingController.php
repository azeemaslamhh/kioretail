...
class SettingController extends Controller
{
    ...
    {       
        if ($request->isMethod('GET')) {
            // get cron_setting from applicationSettings table
            // $settings = applicationSettings::where('setting_name', 'cron_setting')->first();
            // $cron_setting = array();
            // if(!empty($settings->setting_value)) {
            //     $cron_setting = json_decode($settings->setting_value);
            // }
            $cron_settings = DB::table("user_cron_settings")->get();
            $cron_settings_array = array();
            foreach($cron_settings as $crs) { 
                $cron_settings_array[$crs->cron_name] = $crs->cron_value;
            }

            $cron_setting = json_decode(json_encode($cron_settings_array));
            // get time
            $time_1 = [
                '0' => 'Disabled',
                '1' => '1 Min',
            ...
            $pageTitle = trans('settings.cron.page.title');
            $pageDescription = trans('settings.cron.page.desc');
            // breadCrum
            $breadCrum[] = array('title' => trans('breadcrumbs.settings'), 'url' => "javascript:;");
            $breadCrum[] = array('title' => trans('breadcrumbs.cron'), 'url' => "");
            $useBreadCrum="yes";

            return view('backend.setting.crons', compact('pageTitle','pageDescription','breadCrum','cron_setting', 'time_1', 'time_2', 'time_3','time_stats','useBreadCrum'));
        } else {
            // get all input field
            $cron_settings = $request->all();
            unset($cron_settings['_token']);
            unset($cron_settings['_method']);
            unset($cron_settings['btn']);
            // json encode cron setting
            foreach($cron_settings as $key => $cc) { 
                $exists = DB::table("user_cron_settings")->where("cron_name" , $key)->exists();
                if($exists) { 
                    DB::table("user_cron_settings")->where("cron_name" , $key)->update(["cron_value" => $cc]);
                } else { 
                    DB::table("user_cron_settings")->insert(["cron_name" => $key, "cron_value" => $cc]);
                }
                
            }
            // $crons_meta_attributes = json_encode($cron_settings);
            // // get cron_setting from applicationSettings table
            // $setting = applicationSettings::where('setting_name', 'cron_setting')->first();
            // // if application setting is null then create new application setting else update cron_setting
            // if(is_null($setting))
            //     $setting = new applicationSettings();
            // $setting->fill([
            //     'setting_name' => 'cron_setting','setting_value' => $crons_meta_attributes])->save();
            // flash success message
            session()->flash('msg', trans('settings.message.cron_settings_updated'));
            return redirect()->route('setting.crons');
        }
    }
    public function runCronManually(Request $request) 
    {        
       // $send = Artisan::call($request->input('cron')); 
       $rsTasks = Tasks::where('task', $request->cron);
       if($rsTasks->count()==0){
            $objTasks = new Tasks();
            $objTasks->fill(array(
                'user_id' => 1,
            ...
            echo json_encode(array(
                'status'=>'success',
                'message'=>"Program is running in the background, allowing you to perform other tasks.",
                'class'=>'alert alert-success'
            ));
       }else{
        //return 'The Cron "'.$request->cron.'" already in running state';
        echo json_encode(array(
            'status'=>'fail',
            'message'=>'The Cron "'.$request->cron.'" already in running state',
