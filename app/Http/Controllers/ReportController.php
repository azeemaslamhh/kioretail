...
class ReportController extends Controller
{
    ...
    public function calculateAverageCOGS($product_sale_data)
    {
        ...
        foreach ($product_sale_data as $key => $product_sale) {
            ...
            $productResult = Product::select('type', 'product_list', 'variant_list', 'qty_list')->where('id',$product_sale->product_id);
            if($productResult->count()>0){
                $product_data = $productResult->first();
                if($product_data->type == 'combo') {
                    ...
                    foreach ($product_list as $index => $product_id) {
                        ...
                        $sold_qty = (isset($product_sale->sold_qty) && $product_sale->sold_qty>0 && isset($qty_list[$index]) && $qty_list[$index]>0)? $product_sale->sold_qty * $qty_list[$index]:0;
                        foreach ($product_purchase_data as $key => $product_purchase) {
        ...
        foreach ($lims_product_all as $product) {
            ...
            if($warehouse_id == 0) {
                if($product->is_variant) {
                    ...
                    $nestedData['category'] = isset($product->category->name)? $product->category->name:"";
                    //purchase data
    ...
    public function courierReport()
    {
        ...
        return view('backend.report.courier_report', compact('lims_courier_all'));
    }
