...
class HomeController extends Controller
{
    ...
    public function dashboard()
    {
       ...
       $paid_amount = Sale::where('paid_amount','>',0)->sum('paid_amount');
       $grand_total = Sale::where('grand_total','>',0)->sum('grand_total'); 

        //Leopards Courier
        //$couriersResult = DB::table('couriers')->where('is_active',1);
        ...
        $couriersArray = $this->getCourierData($start_date,$endDate);        
        /*if($couriersResult->count()>0){
            $courierData = $couriersResult->select('id','name')->get();
            foreach($courierData as $courierRow){
                $paid_amount_courier = Sale::join('deliveries','sales.id','deliveries.sale_id')->where('deliveries.courier_id',$courierRow->id)->where('sales.paid_amount','>',0)->sum('sales.paid_amount');
                $grand_total_courier = Sale::join('deliveries','sales.id','deliveries.sale_id')->where('deliveries.courier_id',$courierRow->id)->where('sales.grand_total','>',0)->sum('sales.grand_total');
                $couriersArray[] = array(
                    'courier_id'=>$courierRow->id,
                    'courier_name'=>$courierRow->name,
                    'due'=>$grand_total_courier-$paid_amount_courier,
                    'paid_amount'=>$paid_amount_courier,
                );                
            }
            
        }*/
        /*echo '<pre>';
        print_r($couriersArray);
        exit;
        */

        
        /*$grand_total = Sale::where('grand_total','>',0)->whereBetween('sales.created_at',[$start_date,$endate])->sum('grand_total'); 

        $rvReturns = Returns::groupBy('sale_id');
        $return_sale_id = array();
        $sale_return_amount = 0;
        if($rvReturns->count()>0){
            $return_sale_id = $rvReturns->pluck('sale_id');
            $sale_return_amount = Sale::where('paid_amount','>',0)->whereIn('id',$return_sale_id)->whereBetween('sales.created_at',[$start_date,$endate])->sum('paid_amount');             
        } 
        */       
        $saleReturnData = $this->getReturnAmount($start_date,$endDate);
        
        $due_amount = $saleReturnData['due_amount']; //$grand_total- $paid_amount;
        $sale_return_amount = $saleReturnData['sale_return_amount'];
        ...
        if(Auth::user()->role_id <= 2 && isset($_COOKIE['login_now']) && $_COOKIE['login_now']) {
            ...
            //$totalSalesAmount = Sale::where('your_condition_column', 'your_condition_value')->sum('amount');
        }
        else {
            $autoUpdateData = $alertBugEnable = $alertVersionUpgradeEnable = '';
        }
        return view('backend.index', compact('revenue', 'purchase', 'expense', 'return', 'purchase_return', 'profit', 'payment_recieved', 'payment_sent', 'month', 'yearly_sale_amount', 'yearly_purchase_amount', 'alertBugEnable','alertVersionUpgradeEnable','paid_amount','due_amount','sale_return_amount','couriersArray'));
    }
    ...
    public function dashboardFilter($start_date, $end_date)
    {
        ...
        $couriersArray = $this->getCourierData($start_date.' 00:00:00',$end_date.' 23:59:59');
        if(Auth::user()->role_id > 2 && cache()->get('general_setting')->staff_access == 'own') {
            config()->set('database.connections.mysql.strict', false);
            DB::reconnect();
            $product_sale_data = Sale::join('product_sales', 'sales.id','=', 'product_sales.sale_id')
                ...
                ->where('sales.user_id', Auth::id())
                ->whereDate('sales.created_at', '>=' , $start_date.' 00:00:00')
                ->whereDate('sales.created_at', '<=' , $end_date.' 23:59:59')
                ->groupBy('product_sales.product_id', 'product_sales.product_batch_id')
                ->get();
            config()->set('database.connections.mysql.strict', true);
            DB::reconnect();
            ...
            $revenue = Sale::whereDate('created_at', '>=' , $start_date.' 00:00:00')->whereDate('created_at', '<=' , $end_date.' 23:59:59')->sum('grand_total');
            $shipping = Sale::whereDate('created_at', '>=' , $start_date.' 00:00:00')->whereDate('created_at', '<=' , $end_date.' 23:59:59')->where('is_shipping_free',1)->where('shipping_cost','>',0)->sum('shipping_cost');
            $saleReturnData = $this->getReturnAmount($start_date.' 00:00:00',$end_date.' 23:59:59');            
            $return = $saleReturnData['sale_return_amount']; //Returns::whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->where('user_id', Auth::id())->sum('grand_total');
            ...
            $profit = $revenue + $purchase_return - $product_cost-$shipping;            
            $data[0] = $revenue;
            $data[1] = $return;
            $data[2] = $profit;
    ...
    public function calculateAverageCOGS($product_sale_data)
    {
        $product_cost = 0;
        ///Log::info("----------Start---------");
        $data = json_decode($product_sale_data,true);
    ...
    public function  getCourierData($start_date,$endate){       
        $couriersResult = DB::table('couriers')->where('is_active',1);
        $couriersArray = array();
        if($couriersResult->count()>0){
            $courierData = $couriersResult->select('id','name')->get();
            foreach($courierData as $courierRow){
                $paid_amount_courier = Sale::join('deliveries','sales.id','deliveries.sale_id')->where('deliveries.courier_id',$courierRow->id)->where('sales.paid_amount','>',0)->whereBetween('sales.created_at',[$start_date,$endate])->sum('sales.paid_amount');
                $grand_total_courier = Sale::join('deliveries','sales.id','deliveries.sale_id')->where('deliveries.courier_id',$courierRow->id)->where('sales.grand_total','>',0)->whereBetween('sales.created_at',[$start_date,$endate])->sum('sales.grand_total');
                $couriersArray[] = array(
                    'courier_id'=>$courierRow->id,
                    'courier_name'=>$courierRow->name,
                    'due'=>$grand_total_courier-$paid_amount_courier,
    ...
    public function getReturnAmount($start_date,$endate){
        $grand_total = Sale::where('grand_total','>',0)->whereBetween('sales.created_at',[$start_date,$endate])->sum('grand_total'); 
        $paid_amount = Sale::where('paid_amount','>',0)->whereBetween('sales.created_at',[$start_date,$endate])->sum('paid_amount');
        $rvReturns = Returns::groupBy('sale_id');
        $return_sale_id = array();
        $sale_return_amount = 0;
        if($rvReturns->count()>0){
        ...
        $shipping_cost = Sale::where('sales.paid_amount','>',0)->whereBetween('sales.created_at',[$start_date,$endate])->where('sales.is_shipping_free',0)->sum('shipping_cost'); 
        //$rsProduct = Product_Sale::join('sales','product_sales.sale_id','sale_id')->join('products','product_sales.product_id','products.id');
        //Sale::where('sales.paid_amount','>',0)->whereBetween('sales.created_at',[$start_date,$endate])->where('sales.is_shipping_free',0)->sum('	shipping_cost'); 
