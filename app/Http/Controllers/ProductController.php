...
class ProductController extends Controller
{
    ...
    public function productData(Request $request)
    {
        ...
        if (!empty($products)) {
            foreach ($products as $key => $product) {
                ...
                $cateData = '';
                if ($rsCatpro->count() > 0) {
                    $cateData = $rsCatpro->join('categories', 'product_categories.category_id', 'categories.id')->pluck('categories.name')->implode(', ');
                }
                $nestedData['category'] = $cateData; //isset($product->category->name)? $product->category->name :"";
    ...
    public function store(Request $request)
    {
        ...
        if (count($category_ids) > 0) {
            foreach ($category_ids as $categoryID) {
                $objProductCategory = new ProductCategories();
                $categoriesData =  array(
                    'product_id' => $lims_product_data->id,
                    'category_id' => $categoryID
                );
                $objProductCategory->fill($categoriesData);
        ...
        $newCommonClass = new CommonClass();

        $woocommerce_setting = WoocommerceSetting::latest()->first();
        $this->woocommerce = $newCommonClass->woocommerceApi($woocommerce_setting);   

        if ($this->woocommerce != null) {
		try {
            $product_id = $lims_product_data->id;
            $product_row = Product::find($product_id);            
            $catRs = Category::whereIn('id',$category_ids);
            $woocommerce_product_id = $product_row->woocommerce_product_id;
            $categoriesData = array();
            if($catRs->count()>0){
                $categories = $catRs->pluck('woocommerce_category_id');
                foreach($categories as $catgory_id){
                    $categoriesData[] = array('id'=>$catgory_id);
                }
            }

           // Log::info(print_r($categoriesData,true));
            
            //https://kioretail.com/public/images/product/framesi-morphosis-repair-conditioner-250-ml_619ebcbdc64bb.jpeg
            $imageData = array();
            if($product_row->image!=""){
                $imagesArray =  explode(",", $product_row->image);
                foreach($imagesArray as $image){
                    $imageData[] = array('src'=>asset('public/images/product/' . $image));
                }

            }
            $featured = $product_row->featured==1 ? 'featured':""; //(isset($product->featured) && $product->featured != "") ? 1 : 0;
            $woocommerce_data = array(
                'name'=>$product_row->name,                
                'date_modified'=>$product_row->update_at,
               // 'type'=>$product_row->type,                
                //'sale_price'=>(float) $product_row->price,
                'description'=>$product_row->product_details,
                'short_description'=>$product_row->product_details,
                'categories'=>$categoriesData,
                'images'=>$imageData,
                //'featured'=>$featured,
                'sku'=>$product_row->code,
                //'regular_price'=>(string) 
                'sale_price'=>(string) $product_row->price,                
            );
           // Log::info("des price");
           /// Log::info((float) $product_row->price);
            //Log::info(print_r($woocommerce_data,true));
            
            //$woocommerce_setting = WoocommerceSetting::latest()->first();
            //$this->woocommerce = $this->woocommerceApi($woocommerce_setting);
                $product = $this->woocommerce->put('products/' . $woocommerce_product_id,$woocommerce_data);
               /// Log::info("End woocommerce");
        }
        catch (Exception $e) {
            Log::info(Error: "Can't update products: $e");
        }
}
    ...
    public function edit($id)
    {
        ...
        if ($role->hasPermissionTo('products-edit')) {
            ...
            $productCategoriesRs = ProductCategories::where('product_id', $id);
            $categoriesArray = array();
            if ($productCategoriesRs->count() > 0) {
                $categoriesArray =  $productCategoriesRs->pluck('category_id')->toArray();
                /*echo '<pre>';
            ...
            $noOfVariantValue = 0;
            $custom_fields = CustomField::where('belongs_to', 'product')->get();
            return view('backend.product.edit', compact('lims_product_list_without_variant', 'categoriesArray', 'lims_product_list_with_variant', 'lims_brand_list', 'lims_category_list', 'lims_unit_list', 'lims_tax_list', 'lims_product_data', 'lims_product_variant_data', 'lims_warehouse_list', 'noOfVariantValue', 'custom_fields'));
        } else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    { 
        ...
        try{
            Log::info("curl");
        $api_url = 'https://bigbasket.pk/wp-json/wc/v3/products/57415';
        $product_data = array(
            'sku' => '12129999999',            
        );
        $consumer_key = 'ck_07f77f73129dc6e72e236b50113930829936286b';
        $consumer_secret = 'cs_0d3a14ea2b518c372e61994105abf3fa2166f0ab';
        ...
        if (isset($result['id'])) {
            echo 'Product updated successfully. New product ID: ' . $result['id'];
        } else {
        ...
        */
        ...
        //https://bigbasket.pk/wp-json/wc/v3/products
        ...
        if (!env('USER_VERIFIED') && 0) {
            ...
            $woocommerce_product_id = $lims_product_data->woocommerce_product_id; 
            ...
            $category_ids = $request->category_id;
            ProductCategories::where('product_id', $request->input('id'))->delete();
            if (count($category_ids) > 0) {
                foreach ($category_ids as $categoryID) {
                    $objProductCategory = new ProductCategories();
                    $categoriesData =  array(
        ...
        if ($this->woocommerce != null) {
            ...
            try {
                //Log::info("start woocommerce");
                // update product bigbasket //$woocommerce_product_id 
            $product_row = Product::find($request->input('id'));            
            $catRs = Category::whereIn('id',$category_ids);
            $woocommerce_product_id = $product_row->woocommerce_product_id;
            $categoriesData = array();
            if($catRs->count()>0){
    ...
    public function syncProduct($woocommerce_product_id)
    {
        $woocommerce_setting = WoocommerceSetting::latest()->first();
        $this->woocommerce = $this->woocommerceApi($woocommerce_setting);
        if ($this->woocommerce != null) {
            try {
                $product = $this->woocommerce->get('products/' . $woocommerce_product_id);
