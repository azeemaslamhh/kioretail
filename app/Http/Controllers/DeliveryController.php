    ...
    public function index()
    {
        ...
        if($role->hasPermissionTo('delivery')) {
            /*if(Auth::user()->role_id > 2 && config('staff_access') == 'own')
                $lims_delivery_rs = Delivery::orderBy('deliveries.*', 'desc')->where('deliveries.user_id', Auth::id());
            else
                $lims_delivery_rs = Delivery::orderBy('deliveries.id', 'desc');

                $lims_delivery_rs->leftJoin('sales','deliveries.sale_id','sales.id')->leftJoin('users','sales.user_id','users.id')->leftJoin('customers','sales.customer_id','customers.id')->select("deliveries.id","sales.reference_no as sale_reference_no","sales.grand_total","customers.name","customers.phone_number","customers.address","customers.city","users.name as user_name");    

              $lims_delivery_all  = $lims_delivery_rs->get();
            
            $lims_courier_list = Courier::where('is_active', true)->get();
            */
            $lims_courier_list = array();
            $lims_delivery_all = array();
